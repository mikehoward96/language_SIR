...
global end = 2100
	...
	*variable parameters
		...
		global B = 0.05 // 0.05 // tranmissibility. estimated by quasi-MLA
	...
	*initial knowns
		...
		keep state year r_N r_x r_n m f-x_tp1
			...
			*adding in additional years
				local exp = $end - 2018
					...
					foreach var in year m f n x i s W s_ i_ x_ {
					replace `var' = . if e == 1
					}
		...
		*discrete time evolution equations
			by state: replace f = f[_n-1] + ($b * f[_n-1]) - ($d * f[_n-1]) + m if _n != 1
			by state: replace n = n[_n-1] + ($b * n[_n-1]) - ($d * n[_n-1]) if _n != 1
		...
		*iterating model
			forvalues t = $start / $end {
				...
				qui	by state: replace s = s[_n-1] + s_tp1[_n-1] - x_tp1[_n-3] if year == `t' & x_tp1[_n-3] != .	
					qui by state: replace s = s[_n-1] + s_tp1[_n-1] if year == `t' & x_tp1[_n-3] == .
				qui	by state: replace i = i[_n-1] + i_tp1[_n-1] if year == `t'	
					qui	by state: replace i = i[_n-1] if year == `t' & i_tp1[_n-3] == .
				qui	by state: replace x = x[_n-1] + x_tp1[_n-3] if year == `t' & x_tp1[_n-3] != .
					qui by state: replace x = x[_n-1] if year == `t' & x_tp1[_n-3] == .
					
				qui	replace W = ($B * (x/i)) + ($c * $B * (n/i)) if year == `t'
				qui by state: replace i_tp1 = W[_n-4]*s[_n-4] + ($b * n) - ($d * n) if year == `t' & i_tp1[_n-4] != .
					qui replace i_tp1 = ($b * n) - ($d * n) if _n < 4
				qui	replace s_tp1 = ($b * f) - ($d * f) + m if year == `t'
	...
	foreach s of local levels {
		...
		*graph export "graphs\w_projection_`s'.png", replace
		...
		graph export "graphs\projection_all_latent.png", replace
